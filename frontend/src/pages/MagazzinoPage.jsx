...
const MagazzinoPage = () => {
  ...
  const fetchRicambi = async () => {
    try {
      ...
      alert('Errore nel caricamento dei ricambi');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="px-4 sm:px-6 lg:px-8">
      <div className="sm:flex sm:items-center sm:justify-between mb-6">
        <div>
          ...
          <p className="mt-2 text-sm text-gray-700">
            Gestione completa del magazzino ricambi
          </p>
        </div>
        <div className="mt-4 sm:mt-0">
          <Link to="/magazzino/nuovo" className="btn-primary">
            + Nuovo Ricambio
          </Link>
        </div>
      </div>
      ...
      <div className="card overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-12">
            <p className="text-gray-500">Nessun ricambio trovato</p>
          </div>
        ) : (
                  ...
                  <tr key={ricambio.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <span className={`font-medium ${
                        ricambio.quantita <= ricambio.quantita_minima 
                          ? 'text-red-600' 
                          : 'text-gray-900'
                      }`}>
                        {ricambio.quantita}
                      ...
                      {ricambio.quantita <= ricambio.quantita_minima && (
                        <span className="ml-2 badge-danger">Sotto scorta</span>
                      )}
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      €{parseFloat(ricambio.prezzo_acquisto || 0).toFixed(2)}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      €{parseFloat(ricambio.prezzo_vendita || 0).toFixed(2)}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {ricambio.fornitore_nome || '-'}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <Link
                        to={`/magazzino/${ricambio.id}`}
